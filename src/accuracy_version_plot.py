...
def update_plot(version, path):
    ...
    df = pd.read_csv(f'{path}/difference.csv')
    ...
    # Add current version
    accuracy = pd.read_csv(f'{path}/upper_bound.csv')
    ...
    # Save new values
    df.to_csv(f'{path}/difference.csv', index=False)
...
def main(args):
    path = args.path # TODO: environment variable?
    f, ax = update_plot(args.version, path)
    plt.savefig(f'{path}/version_plot.png', dpi=300)
    if args.show:
        plt.show()
        plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default="input/accuracy")
